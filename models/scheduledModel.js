...
function getIncomesByIdAndDate(idUser,startDate,days){
    ...
    let query = `SELECT * FROM scheduledinput WHERE idUser = ? AND idType = 1 AND status = 1 AND  idTypeInput = 1 AND startDate >= ? AND startDate <= ? ;`;
...
function getOutflowsByIdAndDate(idUser,startDate,days){
    ...
    myDate.toString();
    startDate.toString();
    let query = `SELECT * FROM scheduledinput WHERE idUser = ? AND idType = 1 AND idTypeInput = 2 AND status = 1 AND startDate >= ? AND startDate <= ?;`;
    //console.log(query);
    return conexion.runQuery(query,[idUser,myDate,startDate]);
}
////////////////////////////////////////////////////////////////Obtener schedules
...
function getSaldoActualById(id){
    let query = `SELECT
    (
    (SELECT SUM(amount) as 'ingresos' FROM scheduledinput WHERE idTypeInput = 1 AND idUser = ? AND endDate <= CURRENT_DATE AND status = 1)
    -
    (SELECT SUM(amount) as 'egresos' FROM scheduledinput WHERE idTypeInput = 2 AND idUser = ? AND endDate <= CURRENT_DATE AND status = 1)
    ) as 'saldoActual'`;
...
module.exports = {addScheduled,getScheduledById,updateScheduled,changeScheduledStatus,getStatusScheduledById,getSchedulesByUserId,getIncomesByIdAndDate,getOutflowsByIdAndDate,
    getSchedulesByUserIdAndDate,getSaldoActualById, getSaldoFuturoByIdAndDays, getIncomesByUserId, getOutflowsByUserId, getSchedulesIncomesByUserId, getSchedulesOutflowsByUserId}
